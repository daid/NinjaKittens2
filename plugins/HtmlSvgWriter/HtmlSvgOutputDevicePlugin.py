...
class HtmlSvgOutputDevicePlugin(OutputDevicePlugin):
    ...
    def _onSceneChanged(self, node):
        if node.getDecorator(PathResultDecorator.PathResultDecorator):
            if not self.getOutputDeviceManager().getOutputDevice("html_svg_file"):
                self.getOutputDeviceManager().addOutputDevice(HtmlSvgOutputDevice())
                self.getOutputDeviceManager().removeOutputDevice("local_file")

    def stop(self):
        self.getOutputDeviceManager().removeOutputDevice("html_svg_file")

##  Implements an OutputDevice that supports saving to arbitrary local files.
class HtmlSvgOutputDevice(OutputDevice):
    def __init__(self):
        super().__init__("html_svg_file")
    ...
    def requestWrite(self, node, file_name=None, filter_by_machine=False):
        ...
        mesh_writer = HtmlSvgFileWriter.HtmlSvgFileWriter(file_name)
        try:
